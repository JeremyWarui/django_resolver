...
class Command(BaseCommand):
    ...
    def _seed_facility_types(self):
        ...
        for ft in FACILITY_TYPES:
            _, is_new = FacilityType.objects.get_or_create(code=ft["code"], defaults={"name": ft["name"]})
            if is_new:
                created += 1
        self.stdout.write(f"  FacilityType: {created} created, {len(FACILITY_TYPES) - created} already existed")
    ...
    def _seed_priorities(self):
        ...
        for name, rank, response_min, resolution_min in PRIORITIES:
            ...
            if is_new:
                created += 1
        self.stdout.write(f"  Priority: {created} created, {len(PRIORITIES) - created} already existed")
        return priorities
