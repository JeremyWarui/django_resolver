...
### (Optional) Fresh start: reset migrations and rebuild

If you're starting from the aligned models and want a clean schema (recommended for dev):

```bash
# Remove autogenerated migration files (keep __init__.py)
find . -path "*/migrations/*.py" -not -name "__init__.py" -delete
find . -path "*/migrations/*.pyc" -delete

# Drop DB (Postgres) and recreate, or remove sqlite file
# postgres: dropdb resolver_dev && createdb resolver_dev
# sqlite: rm -f db.sqlite3

python manage.py makemigrations
python manage.py migrate

# Optional seeds (idempotent):
...
Expected output:
```
Django version 6.0.3, using settings 'resolver.settings'
