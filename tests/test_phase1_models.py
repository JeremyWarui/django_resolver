...
class TestR2SectionTypeMatch:
    ...
    def test_mismatched_department_raises_validation_error(self, campus, dept):
        from apps.org.models import CampusDepartment, Department, SectionType, Section
...
class TestR3SectionUnique:
    ...
    def test_same_section_type_different_campus_dept_allowed(
        self, campus_dept, section_type
    ):
        ...
        s2 = Section.objects.create(
            campus_department=cd2, section_type=section_type, is_active=True
        )
...
class TestR11TicketLogImmutable:
    ...
    def test_update_raises(self, ticket, user):
        ...
        log = TicketLog.objects.create(ticket=ticket, actor=user, event_type="created")
...
class TestTicketStatusChoices:
    ...
    def test_no_forbidden_statuses(self):
        ...
        actual = {v for v, _ in Ticket.STATUS}
        assert not (
            actual & self.FORBIDDEN
        ), f"Forbidden statuses found: {actual & self.FORBIDDEN}"
