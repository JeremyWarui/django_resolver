...
class TestResolveActiveHolder:
    ...
    def test_cover_assignment_wins_over_standing_hos(self, section, hos_user, cover_user):
        from apps.sla.services.escalation import resolve_active_holder
    ...
    def test_expired_cover_falls_back_to_standing_hos(self, section, hos_user, cover_user):
        from apps.sla.services.escalation import resolve_active_holder
        from apps.accounts.models import RoleAssignment
        section.hos = hos_user
        section.save()
...
class TestEscalationEngine:
    ...
    def _rules(self, ticket):
        from apps.sla.models import EscalationRule
        return list(EscalationRule.objects.filter(priority=ticket.priority).order_by("order"))
    ...
    def test_escalates_to_hos_at_threshold(
        ...
        result = run_escalation_for_ticket(open_ticket, timezone.now(), self._rules(open_ticket))
...
class TestTransferHandler:
    ...
    def test_transfers_assigned_tickets_to_pool(
    ...
    def test_does_not_touch_resolved_tickets(
        self, open_ticket, technician, section
    ):
        from apps.org.services.transfer import transfer_open_tickets
        open_ticket.assigned_to = technician
    ...
    def test_does_not_touch_tickets_in_other_section(
        ...
        st2 = SectionType.objects.create(department=dept2, name="Recruitment", code="REC")
        from apps.org.models import CampusDepartment
        cd2 = CampusDepartment.objects.create(campus=campus_dept.campus, department=dept2)
