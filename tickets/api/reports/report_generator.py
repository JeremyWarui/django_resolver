...
class ExcelReportGenerator:
    ...
    def __init__(self):
        ...
        self.header_fill = PatternFill(start_color="0078D4", end_color="0078D4", fill_type="solid")
...
class TicketLifecycleReport(ExcelReportGenerator):
    ...
    def generate(self, start_date=None, end_date=None, status=None, section_id=None, technician_id=None):
        """
        ...
        # Add headers
        headers = [
            'Ticket No', 'Title', 'Section', 'Facility', 'Raised By',
            'Assigned To', 'Status', 'Created At', 'Resolved At',
            'Resolution Time (hrs)', 'Pending Reason'
        ]
        ws.append([])  # Empty row
        ws.append(headers)
        ...
        ws.append(['Average Resolution Time (hrs):', 
                   round(tickets.filter(resolved_at__isnull=False)
                        .annotate(res_time=ExpressionWrapper(
                            F('resolved_at') - F('created_at'),
                            output_field=DurationField()
                        ))
                        .aggregate(avg=Avg('res_time'))['avg'].total_seconds() / 3600, 2)
                   if tickets.filter(resolved_at__isnull=False).exists() else 0])
...
class TechnicianPerformanceReport(ExcelReportGenerator):
    ...
    def generate(self, start_date=None, end_date=None, technician_id=None):
        ...
        # Add headers
        headers = [
            'Technician', 'Email', 'Total Tickets', 'Resolved', 
            'Pending', 'In Progress', 'Avg Resolution Time (hrs)', 'Avg Rating'
        ]
        ws.append([])
        ws.append(headers)
        self.style_header_row(ws, row_num=4)
        ...
        for tech in technicians:
            ...
            if resolved_with_time.exists():
                avg_seconds = resolved_with_time.aggregate(avg=Avg('res_time'))['avg'].total_seconds()
...
class FacilityHealthReport(ExcelReportGenerator):
    ...
    def generate(self, start_date=None, end_date=None):
        ...
        # Add headers
        headers = [
            'Facility', 'Type', 'Location', 'Total Tickets', 
            'Open Tickets', 'Avg Response Time (hrs)', 'Status'
        ]
        ws.append([])
        ws.append(headers)
        self.style_header_row(ws, row_num=4)
        ...
        for facility in facilities:
            ...
            assigned_tickets = tickets.filter(assigned_to__isnull=False).exclude(status='open')
            avg_response_time = 0
            if assigned_tickets.exists():
                # This is simplified - in production you'd track assignment time
                avg_response_time = round(assigned_tickets.count() * 2.5, 2)  # Placeholder
...
class PendingAnalysisReport(ExcelReportGenerator):
    ...
    def generate(self):
        ...
        pending_tickets = Ticket.objects.filter(status='pending').select_related(
            'section', 'facility', 'raised_by', 'assigned_to'
        ).order_by('-updated_at')
        ...
        # Add headers
        headers = [
            'Ticket No', 'Title', 'Section', 'Facility', 'Assigned To',
            'Created At', 'Updated At', 'Pending Duration (days)', 
        ...
        for ticket in pending_tickets:
            ...
            ws.append([
                ...
                pending_duration,
                ticket.pending_reason if ticket.pending_reason else 'No reason provided',
                priority
            ])
        ...
        ws.append([])
        ws.append(['High Priority (>7 days):', pending_tickets.filter(
            updated_at__lt=timezone.now() - timedelta(days=7)).count()])
        ws.append(['Medium Priority (3-7 days):', pending_tickets.filter(
            updated_at__range=[timezone.now() - timedelta(days=7), 
                              timezone.now() - timedelta(days=3)]).count()])
        ws.append(['Low Priority (<3 days):', pending_tickets.filter(
            updated_at__gte=timezone.now() - timedelta(days=3)).count()])
...
class ComprehensiveReport(ExcelReportGenerator):
    ...
    def generate(self, start_date=None, end_date=None):
        ...
        for wb, prefix in [(tech_wb, 'Tech'), (facility_wb, 'Facility'), (pending_wb, 'Pending')]:
