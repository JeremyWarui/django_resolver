"""
Simple authentication views with role-based authentication strategy.
- Staff roles (technician, admin, manager): password + remember-me
- Users: magic link authentication
"""
...
def simple_auth_login(request):
    """Password login for staff roles only."""
    ...
    # Check if user role is allowed for password login
...
def register_user(request):
    """Register new user with role-based auth method indication."""
    serializer = UserSerializer(data=request.data)

    if serializer.is_valid():
        user = serializer.save()

        # Determine auth method for new user
        if user.role in ['technician', 'admin', 'manager']:
            auth_method = 'password'
        else:
            auth_method = 'magic_link'
