...
def test_admin_cant_assign_ticket_to_technician_not_in_section(api_client, setup_data):
    ...
    plumbing = Section.objects.create(
        name="Plumbing", description="Plumbing systems such as water and piping."
    )
    ...
    payload = {"assigned_to_id": plumber.id, "status": "assigned"}
...
def test_user_cant_update_ticket_status(api_client, setup_data):
    ...
    user2 = CustomUser.objects.create_user(
        username="user2", password="userpass123", email="user2@example.com", role="user"
    )
...
def test_complete_ticket_lifecycle(api_client, setup_data):
    ...
    assignment_response = api_client.patch(
        reverse("ticket-detail", args=[ticket_id]), assignment_payload, format="json"
    )
    ...
    progress_response = api_client.patch(
        reverse("ticket-detail", args=[ticket_id]), progress_payload, format="json"
    )
    ...
    comment_payload = {"text": "Working on the issue. Will be resolved soon."}

    comment_response = api_client.post(
        reverse("ticket-comments", args=[ticket_id]), comment_payload, format="json"
    )
    ...
    resolved_response = api_client.patch(
        reverse("ticket-detail", args=[ticket_id]), resolved_payload, format="json"
    )
    ...
    closed_response = api_client.patch(
        reverse("ticket-detail", args=[ticket_id]), closed_payload, format="json"
    )
    ...
    modified_payload = {"title": "Should Not Change"}
...
def test_admin_workflow_vs_technician_workflow(api_client, setup_data):
    ...
    tech_assign_response = api_client.patch(
        reverse("ticket-detail", args=[ticket_id]), tech_assign_payload, format="json"
    )
    ...
    admin_assign_response = api_client.patch(
        reverse("ticket-detail", args=[ticket_id]), admin_assign_payload, format="json"
    )
